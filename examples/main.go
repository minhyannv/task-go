...
func runSimpleQueueExample(addr, pass string, db int) {
	...
	// 注册订单处理器
	queue.RegisterHandler("order", func(ctx context.Context, t *task.Task) error {
	...
	// 任务1: 邮件
	taskID1, _ := queue.SubmitSimple(ctx, "email", `{
		"to": "user@example.com",
		"subject": "欢迎注册",
		"body": "感谢您的注册！"
	}`)
	...
	// 任务2: 订单
	taskID2, _ := queue.SubmitSimple(ctx, "order", `{
		"order_id": "ORD001",
		"amount": 299.99,
		"items": ["商品A", "商品B"]
	}`)
	...
	// 任务3: 邮件
	taskID3, _ := queue.SubmitSimple(ctx, "email", `{
...
func runDelayedQueueExample(addr, pass string, db int) {
	...
	// 注册提醒处理器
	queue.RegisterHandler("reminder", func(ctx context.Context, t *task.Task) error {
		fmt.Printf("⏰ [延迟队列] 提醒任务: %s\n", t.Payload)
		return nil
	})

	// 注册超时处理器
	queue.RegisterHandler("timeout", func(ctx context.Context, t *task.Task) error {
	...
	// 任务1: 3秒后提醒
	taskID1, _ := queue.SubmitWithDelay(ctx, "reminder", `{
		"type": "meeting",
		"message": "会议将在10分钟后开始",
		"user": "张三"
	}`, 3*time.Second)
	...
	// 任务2: 6秒后超时处理
	taskID2, _ := queue.SubmitWithDelay(ctx, "timeout", `{
		"order_id": "ORD002",
	...
	// 任务3: 1秒后提醒（最先执行）
	taskID3, _ := queue.SubmitWithDelay(ctx, "reminder", `{
		"type": "notification",
		"message": "您有新消息",
		"user": "李四"
	}`, 1*time.Second)
...
func runPriorityQueueExample(addr, pass string, db int) {
	...
	// 注册紧急任务处理器
	queue.RegisterHandler("urgent", func(ctx context.Context, t *task.Task) error {
		fmt.Printf("🚨 [优先级队列] 紧急任务: %s\n", t.Payload)
		return nil
	})

	// 注册普通任务处理器
	queue.RegisterHandler("normal", func(ctx context.Context, t *task.Task) error {
		fmt.Printf("📝 [优先级队列] 普通任务: %s\n", t.Payload)
		return nil
	})

	// 注册低优先级任务处理器
	queue.RegisterHandler("cleanup", func(ctx context.Context, t *task.Task) error {
	...
	// 任务1: 低优先级清理任务
	taskID1, _ := queue.SubmitWithPriority(ctx, "cleanup", `{
	...
	// 任务2: 普通任务
	taskID2, _ := queue.SubmitWithPriority(ctx, "normal", `{
	...
	// 任务3: 紧急任务（最后提交但最先执行）
	taskID3, _ := queue.SubmitWithPriority(ctx, "urgent", `{
	...
	// 再添加一个中等优先级任务
	taskID4, _ := queue.SubmitWithPriority(ctx, "normal", `{
